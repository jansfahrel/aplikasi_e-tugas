...
<title>Kelola Mahasiswa - E-Tugas</title>
...
.form-group input{width:100%;padding:9px 12px;border:1px solid #e2e8f0;border-radius:8px;font-size:13px;font-family:inherit;color:#0f172a;outline:none;transition:.15s}
.form-group input:focus{border-color:#6ee7b7;box-shadow:0 0 0 3px rgba(110,231,183,.2)}
.btn-submit{width:100%;padding:10px;background:#16a34a;color:#fff;border:none;border-radius:8px;font-size:13px;font-weight:600;cursor:pointer;font-family:inherit;margin-top:4px}
...
.badge-role{display:inline-block;padding:3px 10px;background:#f0fdf4;color:#16a34a;border-radius:20px;font-size:11px;font-weight:600;border:1px solid #bbf7d0}
.action-group{display:flex;gap:6px}
.btn-sm{display:inline-flex;align-items:center;gap:5px;padding:6px 11px;border:none;border-radius:7px;font-size:11px;font-weight:600;cursor:pointer;font-family:inherit;transition:.15s}
...
<div class="sidebar">
    ...
    <div class="nav-section">
        ...
        <a class="nav-item active" href="/admin/mahasiswa">🎓 Mahasiswa</a>
...
<div class="main">
    ...
    <div class="topbar">
        <div class="topbar-left">
            ...
            <p>Tambah dan manajemen data mahasiswa</p>
        </div>
        ...
        <div class="page-grid">
            ...
            <div class="card">
                <div class="card-header">
                    <div class="card-header-icon icon-emerald">➕</div>
                    <div>
                        <h3>Tambah Mahasiswa</h3>
                        <p>Isi data untuk menambah akun mahasiswa</p>
                    </div>
                </div>
                ...
                <form action="/admin/users/store" method="POST">
                    ...
                    <div class="form-group">
                        ...
                        <input type="email" name="email" placeholder="nim@mahasiswa.ac.id" value="{{ old('email') }}" required>
                    </div>
                    <div class="form-group">
            ...
            <div class="card">
                <div class="card-header">
                    ...
                    <div>
                        ...
                        <p>Semua akun dengan role mahasiswa</p>
                    </div>
                </div>

                <div class="search-row">
                    <input class="search-input" type="text" placeholder="Cari nama atau email...">
                    <span class="count-badge">{{ $users->count() }} Mahasiswa</span>
                </div>
                ...
                <table>
                    ...
                    @forelse($users as $index => $user)
                        <tr>
                            ...
                            <td><span class="badge-role">Mahasiswa</span></td>
                            <td>
                                <div class="action-group">
                                    <form action="/admin/users/update-role/{{ $user->id }}" method="POST" style="margin:0">
                                        @csrf
                                        <input type="hidden" name="role" value="dosen">
                                        <button type="submit" class="btn-sm btn-warn">↔ Dosen</button>
                                    </form>
                                    <form action="/admin/users/delete/{{ $user->id }}" method="POST" style="margin:0">
                                        @csrf
                                        <button type="submit" class="btn-sm btn-del" onclick="return confirm('Hapus mahasiswa ini?')">🗑 Hapus</button>
                                    </form>
                                </div>
                            </td>
                        </tr>
